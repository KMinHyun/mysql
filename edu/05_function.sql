...
SELECT
	...
	,CAST(1234 AS CHAR(4))
	,CONVERT(1234, CHAR(4))
-- CAST가 과거의 프로그래밍 기법, CONVERT가 현대 기법
;
...
SELECT CONCAT_WS(',', '딸기', '바나나', '수박') AS 과일;
...
-- LEFT(문자열, 숫자)
-- 문자열을 왼쪽부터 숫자 길이만큼 잘라서 반환
SELECT LEFT('abcdef', 2);

-- RIGHT(문자열, 숫자)
-- 문자열을 오른쪽부터 숫자 길이만큼 잘라서 반환
...
-- 방향을 지정해서 문자열 2(원본 문자열)에서 문자열1(제거하고 싶은 문자열)을 제거하여 반환
-- 방향은 LEADING(좌), ROTH(좌우), (TRASLING)(우) 지정 가능.
SELECT TRIM(LEADING 'Bearer'	FROM 'bearer 33frsrsdfsd';

-- SUBSTRING(문자열, 시작위치, 길이)
-- 문자열을 시작 위치에서 길이만큼 발라서 반환
SELECT SUBSTRING('abcdef', 3, 2);

-- SUBSTRING_INDEX(문자열, 구분자, 횟수)
-- 왼쪽부터 구분자가 횟수번째가 나오면 그 이후 문자열을 버림
-- 횟수를 음수로 설정하면, 오른쪽부터 적용
SELECT
	SUBSTRING_INDEX('미어캣_미어켓_테스트', '_', -2)
;
...
-- DATE(데이트 타입의 값) : 데이트 타입의 값을 (YYYY-MM-DD) 형식으로 변환
SELECT DATE(NOW());

-- ADDDATE(날짜1, INTERVAL 숫자 단위)
-- 	날짜1에서 단위(날짜, 시간)에 따라 숫자만큼 더해서 반환
...
-- 	COUNT(컬럼) : 검색 결과의 총 레코드 수를 출력(NULL 미포함)
SELECT
	COUNT(*)
	,COUNT(end_at)
FROM salaries
...
-- 	레코드의 순위를 매겨 반환
-- 	동일한 값이 있는 경우에도 각 행에 고유한 랭클르 부여
-- 	순위보단 검색한 결과의 각 행에 순서를 부여한 것
SELECT
	emp_id
	,salary
